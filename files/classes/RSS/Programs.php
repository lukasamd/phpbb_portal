...
class Rss_Programs extends Rss
{
    public function __construct()
    {
        parent::__construct();
        
        define('MODULE', 'rss_programs');
    ...
    public function execute()
    {
        ...
        $xml_row = $content->createElement('title', Config::getConfig('portal_name') . ' - Ostatnie programy');
        $xml_channel->appendChild($xml_row);
        ...
        $this->Model->getProgramsRss();
        while ($row = $this->Model->fetchResultDB())
        {
            $title = stripslashes($row['title']);
            $url = Config::getConfig('portal_url') . $urls->buildUrl('program', $row);
            $meta = htmlspecialchars(mb_substr(stripslashes(strip_tags($row['meta'])), 0, 400));
            ...
            $xml_row = $content->createElement('category', 'Program');
            $xml_item->appendChild($xml_row);
