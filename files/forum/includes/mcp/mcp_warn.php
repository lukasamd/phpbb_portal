...
function add_warning($user_row, $warning, $send_pm = true, $post_id = 0)
{
  ...
  // END - Reputation system 
	...
	if ($user_row['user_warnings'] >= $config['warnings_ban'])
	{
		...
		user_ban('user', array($user_row['username']), $config['warnings_ban_expire']*1440, false, 0, $user->lang['AUTO_WARN_BAN'], $user->lang['AUTO_WARN_BAN_REASON']);
	}
	// END Ban users by x warnings Mod
}
