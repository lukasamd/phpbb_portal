...
class seo_related {
	...
	* */
	function get($topic_data, $forum_id = false) {
      ...
      // Users avatars MOD
      $users_avatars_ids = array();
  
			while($row = $db->sql_fetchrow($result)) {
        $topic_data[] = $row;
        
        // Users avatars MOD
        $users_avatars_ids[] = $row['topic_last_poster_id'];
      }
      
      
      // Users avatars MOD
      $users_avatars = getUsersAvatarsByIds($users_avatars_ids);
      ...
      foreach ($topic_data as $row)
      {
            ...
            //-- mod : Who posted ------------------------------------------------------------
            //-- add
            			'U_WHO_POSTED'			=> append_sid("{$phpbb_root_path}posting.$phpEx", "mode=who_posted&amp;t=$related_topic_id&amp;f=$related_forum_id"),
            //-- fin mod : Who posted --------------------------------------------------------
            ...
            // Users avatars MOD
            'LAST_POSTER_LINK'  => get_username_string('profile', $row['topic_last_poster_id'], $row['topic_last_poster_name'], $row['topic_last_poster_colour']),
            'LAST_POSTER_AVATAR'  => $users_avatars[$row['topic_last_poster_id']],
