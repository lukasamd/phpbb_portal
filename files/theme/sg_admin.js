...
{
    return confirm('Czy na pewno chcesz usunąć dany element?');
}
...
{
    document.getElementById('add_image_src').src = DIR_IMAGES + encodeURI(newimage);
}
...
{ 
    ...
    var pollOption = 5;
    $('input[name$="addOption"]').on('click', function(ev)
    {
        ev.preventDefault();
        $('#pollData tr:last').after('<tr><td>Opcja ' + pollOption + '<br />&nbsp;</td><td><input type="text" name="pollOptions[]" value="" class="textbox" style="width:300px"><br />&nbsp;</td></tr>');
        pollOption++;
    });
    
    
    /* Aktualizacja paneli w locie */
    ...
    $('.chooseList').sortable(
    {
        ...
        update: function () 
        {
            ...
            i = 0;
            $('#info').load('index.php?p=panels_updater&type=' + panelType + '&'+order);
        ...
        receive: function () 
        {
            ...
            if (pdata == 'header') { var psidetext = 'Góra'; }
            if (pdata == 'left') { var psidetext = 'Lewa'; }
            if (pdata == 'right') { var psidetext = 'Prawa'; }
            if (pdata == 'footer') { var psidetext = 'Dół'; }
            ul.find('.pside').each(function() 
            {
                $(this).text(psidetext);
            });
            $('#info').load('index.php?p=panels_updater&type=' + panelType + '&side='+pdata+'&'+order);
        }
    });
