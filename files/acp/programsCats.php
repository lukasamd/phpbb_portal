...
if ($action == 'save')
{
    ...
    {
        $sql = 'INSERT INTO ' . DB_SOFTWARE_CATS . ' ' . $db->sql_build_array('INSERT', $sql_arr);
        $db->sql_query($sql);
    ...
    if ($catData)
    {
        ...
        if ($newCat)
        {
            $sql    = 'UPDATE ' . DB_SOFTWARE . '
              SET cat = ' . $newCat . '
    					WHERE cat = ' . $cat_id;
            $result = $db->sql_query($sql);
            $sql    = 'DELETE FROM ' . DB_SOFTWARE_CATS . '
              WHERE cat_id = ' . $cat_id;
            $result = $db->sql_query($sql);
        ...
        <form name="inputform" method="post" action="<?php echo $formAction; ?>" onSubmit="return ConfirmDelete()">
            <table align="center" cellpadding="0" cellspacing="0">
                <tr>
                    <td class="tbl">Wybierz kategorię, do której mają być przeniesione programy.</td>
                </tr>
                <tr>
                    <td class="tbl">
...
else
{
    if ($action == 'edit')
    {
        ...
        $theme->panelOpen('Edycja istniejącej kategorii programów');
    }
    else
    {
        $cat_title   = '';
        ...
        $theme->panelOpen('Dodawanie nowej kategorii programów');
    }
    ...
    <?php
    $theme->panelClose();
    $theme->panelOpen('Istniejące kategorie programów');
    ...
    while ($row = $db->sql_fetchrow($result))
    {
        $categories[$row['cat_id']] = array(
            ...
            'img_edit'    => buildIconLink('edit', $FILE_SELF . "?action=edit&amp;cat_id={$row['cat_id']}"),
            'img_delete'  => buildIconLink('delete', $FILE_SELF . "?action=delete&amp;cat_id={$row['cat_id']}"),
    ...
    if (!empty($categories))
    {
        ...
        {
            ?>
        <h3 class="error center">W bazie nie ma jeszcze żadnej kategorii programów</h3>
        <?php
    }
    $theme->panelClose();
}
